...
var saveItem = function saveItem( data , collectionName , dbName ){
	...
	if( db === undefined ) // if not established set the timer to 5 secs
		...
		time = 0;	// dont wait coz database object is not undefined
	...
	if( !isCorrectFormat ){
		...
		setTimeout( function onTimeout( ){ 
			db= connection.open( );
			db.collection( collectionName , function onSave( error , collection ){
					collection.insert( data , { w:1 } , function onSave( error ){
						if( error )
							return logger.error( Error + " via command line..." );
						logger.insert( "The data is inserted via command line..." );
					} );
				} );
