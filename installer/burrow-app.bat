...
:wgetInstall32
	if not exist "%USERPROFILE%\Documents\GridInstallers\wget-1.11.4-1-setup.exe" goto wgetDownload32
		...
		:checkJava32
			set file="%USERPROFILE%\Documents\GridInstallers\jdk-8u25-windows-i586_2.exe"
			set fileSize=164902816

			for /f "usebackq" %%A IN ('%file%') DO set size=%%~zA

				if %size% LSS %fileSize% (
					del "%USERPROFILE%\Documents\GridInstallers\jdk-8u25-windows-i586_2.exe"
			goto javaDownload32
				) else if %size% GTR %fileSize% (
					del "%USERPROFILE%\Documents\GridInstallers\jdk-8u25-windows-i586_2.exe"
			goto javaDownload32
...
:javaDownload32
	if exist "%USERPROFILE%\Documents\GridInstallers\jdk-8u25-windows-i586_2.exe" goto checkJava32
...
:wgetInstall64
	if not exist "%USERPROFILE%\Documents\GridInstallers\wget-1.11.4-1-setup.exe" goto wgetDownload64
		...
		:checkJava64
			set file="%USERPROFILE%\Documents\GridInstallers\jdk-8u25-windows-x64.exe"
			set fileSize=177856928

			for /f "usebackq" %%A IN ('%file%') DO set size=%%~zA

				if %size% LSS %fileSize% (
					del "%USERPROFILE%\Documents\GridInstallers\jdk-8u25-windows-x64.exe"
			goto javaDownload64
				) else if %size% GTR %fileSize% (
					del "%USERPROFILE%\Documents\GridInstallers\jdk-8u25-windows-x64.exe"
			goto javaDownload64
...
:javaDownload64
	if exist "%USERPROFILE%\Documents\GridInstallers\jdk-8u25-windows-x64.exe" goto checkJava64
...
:burrowExist
	if not exist "%USERPROFILE%\Documents\burrow-app" goto appCheck
		...
		git clone -b feature/improvements "https://github.com/volkovasystems/burrow-app.git" temporaryburrow-app
...
:burrowNotExist
	if exist "%USERPROFILE%\Documents\burrow-app" goto appCheck
		...
		echo Cloning Burrow-App repository from github.com, please wait...
		::git clone -b develop "https://github.com/volkovasystems/burrow-app.git"
		git clone -b feature/improvements "https://github.com/volkovasystems/burrow-app.git" burrow-app
