...
var getValue = function getValue( value, collectionName, databaseName, databaseHost, databasePort, callback ){
	...
	connect( databaseName, databaseHost, databasePort, function onConnect( databaseName ){
		...
		if( !isCorrectFormat ){
			...
			databaseName.collection( collectionName , function onInsert( error , collection ){
				collection.find( value, { "_id" : 0  , "__v" : 0 } ).toArray(  function onGet( error, items ){ 
					if( error ){
						...
						disconnect( databaseName );
						callback ( "Retrieved: " + JSON.stringify( items ) );
					}
				} );
			} );
		}
...
getValue( "{\"abc\":\"12345\"}", "testpao", "reinvent1", "127.0.0.1", "27017",
	function( message ){
		console.log( message );
	} );