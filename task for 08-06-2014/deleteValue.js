...
var deleteValue = function deleteValue( value, collectionName, databaseName, databaseHost, databasePort, callback ){
	...
	connect( databaseName, databaseHost, databasePort, function onConnect( databaseName ){
		...
		if( !isCorrectFormat ){
			...
			databaseName.collection( collectionName , function onInsert( error , collection ){
				collection.remove( value, function onRemove( error, result ){
					if( error ){
...
deleteValue( "{\"abc\":\"12345\"}", "testpao", "reinvent1", "127.0.0.1", "27017",
	function( message ){
		console.log( message );
	} );